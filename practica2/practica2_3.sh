...
then
    if [ -e "$1" ] && [ -f "$1" ]    #se comprueba si stat dice que es un fichero 
                                    #regular y si existe
    then
        chmod ug+x "$1"                    #de ser así, se da permiso de ejecución al grupo y propietario del fichero
        echo `stat "$1" --printf=%A`       #se stat imprime por la salida estandar los permisos
    else
