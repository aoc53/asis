...
then
    echo "El numero de ficheros y directorios en "$directorio""             #se saca por la salida estandar el número de 
    echo "es de "$(ls -l "$directorio" | grep "^-" | wc -l)" y"             #lineas de la salida ls -l que empiezan por
    echo ""$(ls -l "$directorio" | grep "^d" | wc -l)", respectivamente"    #- (ficheros) y el numero de lineas que 
